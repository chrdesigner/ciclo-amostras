...
class acf_brazilian_city_field extends acf_field {
    ...
    function create_field( $field )
    {
        ...
        //Carregando Estados
        $states = array("__" => "");
        $statesResults = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."brazilian_states ORDER BY name ASC");
    ...
    {
        wp_register_script('acf-brazilian-city', $this->settings['dir'] . 'assets/js/brazilian-city.js', array('acf-input'), $this->settings['version']);
        wp_register_script('acf-input-chosen', $this->settings['dir'] . 'assets/js/chosen.jquery.min.js', array('jquery'), $this->settings['version']);
        wp_register_style('acf-input-chosen', $this->settings['dir'] . 'assets/css/chosen.min.css', array(), $this->settings['version']);
    ...
    {
        global $wpdb;
        $cities_results = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."brazilian_cities WHERE state_id ='".$state_id."' ORDER BY name ASC");
        $cities = array(); 
    ...
    {
        global $wpdb;
        $city = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."brazilian_cities WHERE id = '".$city_id."'");
    ...
    {
        global $wpdb;
        $state = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."brazilian_states WHERE id = '".$state_id."'");
...
add_action('wp_ajax_nopriv_gt_list_state_cities', 'get_list_state_cities');
/**
